    ...
    for i in st.session_state.uploaded_files:
        ...
        if i == st.session_state.sample_plot:
            ...
            ax.fill_between(x, max(df_data['11B']), where=(
                x < sig_end) & (x > sig_str), alpha=0.5)
            ax.fill_between(x, max(df_data['11B']), where=(
                x < bac_end) & (x > bac_str), alpha=0.5)
...
def bacground_sub(factorSD, factor_B11):
    ...
    for i in st.session_state.uploaded_files:
        ...
        df_bacground_mean = df_data[bac_str:bac_end].mean()
        df_signal = df_data[sig_str:sig_end]
...
if len(st.session_state.uploaded_files) != 0:
    ...
    bac_str, bac_end = st.slider('Select bacground', 0, 200, (5, 70))
    sig_str, sig_end = st.slider('Select signal', 0, 200, (95, 175))
    ...
    sample_correction = st.selectbox(
        'Which type is your choosed standard?',
        ('A', 'B', 'C', 'D'))

    default_reg_level = 4
    st.session_state.regress_level = st.number_input('insert your regression level (4 is recommended)', step=1, value=default_reg_level, format='%X'
                                                     )
    ...
    factor_iso = regression(x, y_isotope,
                            number_iso,
                            st.session_state.regress_level if "regress_level" in st.session_state else default_reg_level,
...
# try try try
    ...
    # if st.button('clear laser data'):
    #     st.session_state.laser_file = []
    ...
    # if 'laser_file' in st.session_state:# and st.session_state.laser_file is not None or len(st.session_state.laser_file) != 0:
    #     laser_file = st.session_state.laser_file
    # else:
    #     st.session_state.laser_file = st.file_uploader("Choose a laser file", type='csv')

    #st.write(st.session_state.df_Laser)



    # st.session_state.laser_file = st.file_uploader("Choose a laser file", type='csv' ,accept_multiple_files=True)
    # st.write(st.session_state.laser_file)

    # if "laser_file" in st.session_state and len(st.session_state.laser_file) > 0:
    #     st.session_state.df_Laser = pd.read_csv(st.session_state.laser_file[0])

        df_Laser_part1 = st.session_state.df_Laser[st.session_state.df_Laser[' Laser State']
                                == 'On'].iloc[:, [13, 20, 21]]
        df_Laser_part2 = st.session_state.df_Laser[st.session_state.df_Laser[' Sequence Number'].notnull()].iloc[:, [
                1, 4]]

        df_Laser_res = pd.concat([df_Laser_part2.reset_index(
                drop=True), df_Laser_part1.reset_index(drop=True)], axis=1)
        ...
        if spot_shape == 'circle':
            df_map1[' Spot Size (um)'] = df_Laser_res[' Spot Size (um)']

            ref = ((df_map1[df_map1['filename'].str.contains(sample_correction)][' Spot Size (um)']/2)**2).mean()

        if spot_shape == 'squre':
            dia = df_map1[df_map1['filename'].str.contains(sample_correction)][' Spot Size (um)']
            spotsize = dia.str.split(' ').str[0].apply(lambda x: float(x))
            df_map1[' Spot Size (um)'] = spotsize
            ref = ((df_map1[df_map1['filename'].str.contains(sample_correction)][' Spot Size (um)'])**2).mean()
        ...
        default_reg_level_B = 4
        st.session_state.regress_level_B = st.number_input('insert your regression level for [B] (4 is recommended)', step=1, value=default_reg_level, format='%X'
                                                     )        
        factor_B = regression(x, y_11B, number_trace,
                        st.session_state.regress_level_B if "regress_level_B" in st.session_state else default_reg_level_B, 
                        df_data.index.to_numpy()
                        )
        df_map1['factor_B'] = factor_B
        ...
        depth_ratios = []
        for i in df_map1['filename'].str.contains('A'):
            if i == True:
                depth_ratio = 1 
            else:
        ...
        df_map1['depth_correction'] = depth_ratios
    
        # #use spot diameter, depth and signal intensity to calculate [B]


        if spot_shape == 'circle':
            df_map1['[B]_corrected'] = df_map1['11B']*df_map1['factor_B'] * \
                (ref / ((df_map1[' Spot Size (um)']/2)**2) / depth_ratios)
        if spot_shape == 'squre':
            df_map1['[B]_corrected'] = df_map1['11B']*df_map1['factor_B'] * \
                (ref / ((df_map1[' Spot Size (um)'])**2) / depth_ratios)                
        ...
        if trace_file == 'No':
            st.session_state.df_all = df_map1
            ...
            if "trace" in  st.session_state and len(st.session_state.trace) > 0:
                ...
                st.session_state.df_all = df_map1.merge(df_trace, on=' Sequence Number')
...
# start_num, end_num = st.slider('Select rows to plot', 0, len(df_all), (12, 15))
